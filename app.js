// v5.5.1 – Anonymous can only CREATE results; all other writes admin-only.
...
function updateAdminUI(user){
  ...
  const allowed = !!(user && ((ADMIN_UIDS.includes(user.uid)) || (user.email && ADMIN_EMAILS.includes(user.email))));
  isAdmin = allowed;
...
async function saveScore(){
  ...
  try {
    if (isAdmin) {
      ...
      // Anonymous viewers: CREATE only (will fail if doc exists)
      await ref.create(payload);
      alert('Match submitted');
    }
  } catch (e) {
    const msg = String(e && e.message || e || '');
    if (!isAdmin && /already exists/i.test(msg)) {
...
async function renamePlayerAcrossMatches(originalName, newName) {
  ...
  try {
    ...
    try {
      ...
      if (snap.exists) { const data = snap.data() || {}; const newRef = db.collection(PLAYERS_COL).doc(newName); await newRef.set(data, { merge: true }); await oldRef.delete(); }
    } catch (e) { console.warn('Phone mapping move failed:', e); }
  } catch (err) {
...
async function uploadScheduleToFirestore(){
  ...
  async function clearCollection(col){
    ...
    for(const group of chunks){ const batch = db.batch(); group.forEach(doc => batch.delete(doc.ref)); await batch.commit(); }
  }
  await clearCollection(MATCHES_COL);
  await clearCollection(RESULTS_COL);
  const batch = db.batch();
  arr.forEach((m, idx) => { const id = String(idx); const ref = db.collection(MATCHES_COL).doc(id); batch.set(ref, { round: m.round, team1: m.team1, team2: m.team2 }); });
  await batch.commit();
...
async function deleteAllResults(){
  ...
  await batch.commit(); alert('All results deleted');
}
